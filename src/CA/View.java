...
//		}
	...
	public ArrayList addViemInCome(Map<Integer,Double> viewInCome1,Map<Integer,Double> viewInCome2,Map<Integer,Double> viewInComeCom,Map<Integer,Double> seeExitInCome){
		...
		for(int i=0;i<viewInCome1.size();i++){
			...
			dir.setDirection(i);
			dir.setInCome(viewInCome1.get(i)+viewInCome2.get(i)+viewInComeCom.get(i));
			viewInComeAll.add(dir);
		}
		return viewInComeAll;
	
	}
	public ArrayList<Direction> outPutDirection(People people){
		...
		for(int i=0;i<viewInComeAll.size();i++){
			if(viewInComeAll.get(i)==null){
				viewInComeAll.remove(i);
				--i;
			}
		}
		for(int i=0;i<viewInComeAll.size()-1;i++){
			for(int j=0;j<viewInComeAll.size()-1;j++){
				if(viewInComeAll.get(j).getInCome()<viewInComeAll.get(j+1).getInCome()){
					Direction d1=viewInComeAll.get(j);
					Direction d2=viewInComeAll.get(j+1);
					viewInComeAll.set(j, d2);
					viewInComeAll.set(j+1,d1);
				}
			}
		}
		return viewInComeAll;
